...
I have been curious about asemic writing for some time. In this notebook I adapt the code of Georgios Karamanis to create asemic glyphs: check https://github.com/gkaramanis/aRtist/blob/main/asemic/asemic.R
...
Set the parameters for creating the glyphs, that is, the number of glyphs (corresponding to letters), the number of points to generate each glyph (used by the splines function), and the cutoff for the thickness of the lines:
```{r}
...
Plot the text while grouping by column to create the impression of top-bottom writing:
```{r}
ggplot(atext_bd, 
       aes(x = rx, 
           y = ry, 
...
Plot the text while grouping by row to create the impression of right-left writing:
```{r}
ggplot(atext_bd, 
       aes(x = rx, 
           y = ry, 
